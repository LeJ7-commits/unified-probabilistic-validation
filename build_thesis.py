...
def find_pandoc() -> str | None:
    """
    Find pandoc executable — checks PATH first, then common Windows install
    locations. Returns the full path string or None if not found.
    """
    import shutil
    ...
    # 2. Common Windows locations after winget install
    candidates = [
        Path(os.environ.get("LOCALAPPDATA", "")) / "Pandoc" / "pandoc.exe",
        Path(os.environ.get("APPDATA", ""))      / "Pandoc" / "pandoc.exe",
...
PANDOC_EXE: str | None = None   # set by check_pandoc()


def check_pandoc() -> bool:
    """Check pandoc is available, set PANDOC_EXE global."""
    global PANDOC_EXE
    ...
    try:
        ...
        print(f"  pandoc: {version}  ({PANDOC_EXE})")
        return True
    except Exception as e:
        print(f"  [ERROR] pandoc found at {PANDOC_EXE} but failed to run: {e}")
        return False
