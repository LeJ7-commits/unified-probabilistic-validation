"""
experiments/run_010_entsoe_solar.py
=====================================
Probabilistic validation diagnostics for ENTSO-E solar PV (Germany, 2020-2026).
Nighttime hours excluded during build step.
"""
...
DATA_DIR  = REPO_ROOT / "data" / "derived_entsoe_solar"
OUT_DIR   = REPO_ROOT / "experiments" / "run_010_entsoe_solar"
ALPHA     = 0.1
MODEL_ID  = "entsoe_solar_de"


def main() -> None:
    print("=" * 60)
    print("Running ENTSO-E Solar (Germany) diagnostics")
    print("=" * 60)

    y       = np.load(DATA_DIR / "entsoe_solar_y.npy").astype(float)
    lower   = np.load(DATA_DIR / "entsoe_solar_lo_base_90.npy").astype(float)
    upper   = np.load(DATA_DIR / "entsoe_solar_hi_base_90.npy").astype(float)
    samples = np.load(DATA_DIR / "entsoe_solar_samples.npy").astype(float)
