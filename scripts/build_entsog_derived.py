"""
scripts/build_entsog_derived.py
================================
...
Distribution reconstruction method:
    ...
    notebooks/02_entsog_feasibility.ipynb, where it achieved the best
...
Outputs (saved to data/derived_full/):
    entsog_y.npy             realised values (n,)
    entsog_yhat.npy          point forecasts (n,)
    entsog_scale.npy         robust scale estimate per observation (n,)
    entsog_lo_base_90.npy    lower bound of 90% base interval (n,)
    entsog_hi_base_90.npy    upper bound of 90% base interval (n,)
    entsog_samples.npy       MC samples from Normal(yhat, scale) (n, n_samples)
...
def build_derived(
    ...
    *,
    ...
    ----------
    csv_path : Path
        Raw CSV (entsog_full.csv or entsog_sample_90days.csv).
    out_dir : Path
    ...
    meta = {
        ...
        "reconstruction_method": (
            ...
            "Selected based on feasibility analysis in notebooks/02_entsog_feasibility.ipynb."
        ),
        "bucket_definitions": {
            "0_night":     "00:00-05:59",
    ...
    print(f"[entsog] n_raw={n_raw}, n_clean={n_clean}, n_eval={n_eval}, "
...
if __name__ == "__main__":
    ...
    configs = [
        (raw_dir / "entsog_full.csv",          repo_root / "data" / "derived_full"),
        (raw_dir / "entsog_sample_90days.csv",  repo_root / "data" / "derived_dev"),
    ]

    for csv_path, out_dir in configs:
        if not csv_path.exists():
            print(f"[entsog] Skipping missing file: {csv_path}")
            continue
